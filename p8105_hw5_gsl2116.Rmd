...
In addition to location and report date, this dataset includes information on the race, age, and sex of each victim. Of the `r format(nrow(homicides), big.mark=",", scientific=FALSE)` victims, `r format(nrow(subset(homicides, victim_sex == "Female")), big.mark=",", scientific=FALSE)` (`r round(nrow(subset(homicides, victim_sex == "Female"))/nrow(homicides)*100, digits = 2)`%) were female and `r format(nrow(subset(homicides, victim_sex == "Male")), big.mark=",", scientific=FALSE)` (`r round(nrow(subset(homicides, victim_sex == "Male"))/nrow(homicides)*100, digits = 2)`%) were male. The average age of all victims was `r round(mean(homicides$victim_age, na.rm = TRUE), digits=2)`, with the oldest and youngest victims being `r max(homicides$victim_age, na.rm = TRUE)` and `r min(homicides$victim_age, na.rm = TRUE)` years old, respectively. A further breakdown of victims by race, age, and sex, is included below:

```{r victim descriptions, echo=FALSE}
kable(
  group_by(homicides, victim_race) %>%
  summarise(
    total = format(n(), big.mark=",", scientific=FALSE),
    average_age = round(mean(victim_age, na.rm = TRUE), digits = 2),
    male = format(sum(victim_sex == "Male", na.rm = TRUE), big.mark=",", scientific=FALSE),
    female = format(sum(victim_sex == "Female", na.rm = TRUE), big.mark=",", scientific=FALSE),
    sex_unknown = format(sum(victim_sex == "Unknown", na.rm = TRUE), big.mark=",", scientific=FALSE)
  ))
```
