...
This dataset contains details on `r format(nrow(homicides), big.mark=",", scientific=FALSE)` homicides occurring across `r nrow(homicides_summary)` major US cities between `r min(homicides$reported_date)` and `r max(homicides$reported_date)`. 

The city with the highest number of total homicides was `r homicides_summary$city_state[match(max(homicides_summary$total_homicides), homicides_summary$total_homicides)]`, where there were a total of `r max(homicides_summary$total_homicides)` homicides over the decade. The city with the lowest number of total homicides was `r homicides_summary$city_state[match(min(homicides_summary$total_homicides), homicides_summary$total_homicides)]`, where there were a total of `r min(homicides_summary$total_homicides)` homicides over the decade. 
